...
Variant of SnippetList which sends the result of running a snippet directly to a player.

This can become a subclass of SnippetList if we refactor it to store class variable values in the Registry.
...
PlayerSnippetList {
	...
	*new { | path |
	...
	getSnippetsFromSource {
		...
		all = PlayerSnippet.readAll(path);
		all do: { | s |
			switch (s.type,
