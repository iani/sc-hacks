...
* Other TODOs
  ...
  :END:
** TODO Move synth of linked player to appropriate group when linked
   :PROPERTIES:
...
#+BEGIN_SRC sclang
  ...
  //Link source and effect player. This statement should work in any order.
  \source3 *> \effect3;
#+END_SRC
...
This is because the synth of the source is not moved to the appropriate group after the players are linked.  Therefore issue a "changed" notification when linking, and catch it to move the synth of the writing player to a group before the reading player. 
